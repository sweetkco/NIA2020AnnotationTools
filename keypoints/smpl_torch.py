...
class SMPLModel(Module):
  def __init__(self, device=None, model_path='./model.pkl'):
    ...
    self.shapedirs = torch.from_numpy(params['shapedirs']).type(torch.float64)[:,:,:10]
...
def test_gpu(gpu_id=[0]):
  ...
  model = SMPLModel(device=device, model_path='./model/SMPL_f_lbs_300_207_0_v1.0.0_p3.pkl')
